...
-- Consumer-permissioned M-Pesa statement uploads (NemoScore Phase 2).
-- Raw parsed rows live here for provenance + receipt-level dedupe; each row is
-- also projected into `transactions` (channel = 'MPESA') for the base scorer.
...
-- Score-change alert feed (portal Alerts page) + per-customer preferences.
-- Mirrored in migrations/2026_07_score_alerts.sql for existing DBs.
...
CREATE INDEX idx_score_alerts_customer ON score_alerts (customer_id, created_at DESC);
